...
- `autoConvertComponentClass` (Default false). Converts the classes in `Component.js` (or .ts) file by default, if it extends from an imported class. 
  Consider this option if your Component file is not converted, and you are getting the error "TypeError: Class constructor Component cannot be invoked without 'new'" during runtime of your application. 
  Use `@nonui5` if there are multiple classes in a component file which extend from an import.
